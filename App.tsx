...
const App: React.FC = () => {
  ...
  // VIEW MODE: INTRO, MAP, or BUNKER
  const [viewMode, setViewMode] = useState<'INTRO' | 'MAP' | 'BUNKER'>('INTRO');
  ...
  useEffect(() => {
    if (missions.length === 0) {
        ...
        // FISK TRILOGY (Stacked Coordinates)
  ...
  const handleMissionMove = useCallback((id: string, newPos: Coordinates) => {
    setMissions(prev => prev.map(m => m.id === id ? { ...m, position: newPos } : m));
  }, []);
    ...
    setMissions(prev => {
        ...
        if (status === MissionStatus.COMPLETED) {
            updatedMissions = updatedMissions.map(m => {
                if (m.status === MissionStatus.LOCKED) {
                    ...
                    if (allParentsDone) return { ...m, status: MissionStatus.AVAILABLE };
                }
                return m;
            });
        }
        if (status !== MissionStatus.COMPLETED) {
             updatedMissions = updatedMissions.map(m => {
                ...
                if (hasUnfinishedParent && m.status !== MissionStatus.COMPLETED) return { ...m, status: MissionStatus.LOCKED };
                return m;
            });
        }
          ...
          return prev.map(m => {
              if (m.id === childId && !m.dependencies.includes(parentId)) {
                  ...
                  return { 
                      ...
                      status: isParentDone ? m.status : MissionStatus.LOCKED 
                  };
              }
              return m;
          });
  ...
  const handleIntroComplete = (chosenMode: GameMode) => {
